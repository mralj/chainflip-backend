macro_rules! generate_vote_storage_tuple_impls {
    ($module:ident: ($($t:ident),*$(,)?)) => {
        pub(crate) mod $module {
            ...
            use super::super::{private, VoteStorage, PartialOrVote, SharedDataHash, VoteComponents};
            ...
            impl<$($t: VoteStorage),*> VoteStorage for ($($t,)*) {
                ...
                fn components_into_vote<F: FnMut(SharedDataHash) -> Result<Option<Self::SharedData>, CorruptStorageError>>(
                    ...
                    mut f: F,
                ) -> Result<Option<(Self::Properties, PartialOrVote<Self::PartialVote, Self::Vote>)>, CorruptStorageError> {
                    match vote_components {
                        $(
                            VoteComponents {
                                ...
                                )?.map(|(properties, partial_vote_or_vote)| {
                                    (
                                        CompositeVoteStorageEnum::$t(properties),
                                        match partial_vote_or_vote {
                                            PartialOrVote::PartialVote(partial_vote) => PartialOrVote::PartialVote(CompositeVoteStorageEnum::$t(partial_vote)),
                                            PartialOrVote::Vote(vote) => PartialOrVote::Vote(CompositeVoteStorageEnum::$t(vote)),
                                        },
                                    )
                                }))
                            },
