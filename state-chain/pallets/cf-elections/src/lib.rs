...
pub mod pallet {
	...
	use election_bitmap_components::ElectionBitmapComponents;
	use electoral_system::{
		ConsensusStatus, ElectionReadAccess, ElectionWriteAccess, ElectoralSystem,
		MultiElectionReadAccess, MultiElectionWriteAccess,
	};
	use frame_support::{sp_runtime::traits::BlockNumberProvider, StorageDoubleMap as _};
	use itertools::Itertools;
	use sp_std::collections::btree_map::BTreeMap;
	use vote_storage::{PartialOrVote, SharedDataHash, VoteComponents, VoteStorage};

	pub const BLOCKS_BETWEEN_CLEANUP: u64 = 128;
	...
	impl<Extra> From<ElectionIdentifier<Extra>> for UniqueMonotonicIdentifier {
		fn from(value: ElectionIdentifier<Extra>) -> Self {
			value.0
		}
	}
	...
	pub trait Config<I: 'static = ()>: Chainflip {
		...
		type ElectoralSystem: ElectoralSystem<Context = ()>;
	}
	...
	type SharedData<T: Config<I>, I: 'static = ()> = StorageMap<
		_,
		Identity, /* TODO: Talk to Dan about safety */
		SharedDataHash,
	...
	type IndividualComponents<T: Config<I>, I: 'static = ()> = StorageDoubleMap<
		...
		T::ValidatorId,
		(
			<<T::ElectoralSystem as ElectoralSystem>::Vote as VoteStorage>::Properties,
			<<T::ElectoralSystem as ElectoralSystem>::Vote as VoteStorage>::IndividualComponent,
		),
		OptionQuery,
	>;
	...
	/// Stores any persistent state the electoral system needs.
	#[pallet::storage]
	...
	type ElectionProperties<T: Config<I>, I: 'static = ()> = StorageMap<
		_,
		Twox64Concat,
		ElectionIdentifier<<T::ElectoralSystem as ElectoralSystem>::ElectionIdentifierExtra>,
	...
	/// Stores if we have detected corrupt storage in this pallet, and therefore have pausing all
	/// elections and voting.
	#[pallet::storage]
	type CorruptStorage<T: Config<I>, I: 'static = ()> = StorageValue<_, bool, ValueQuery>;

	/// Stores if elections are being run. When paused the ElectoralSystem's `on_finalize` will not
	/// be called, and the pallet will not ask validators for any more votes. We use "Active" and
	/// not the inverse "Paused", so the default is to be paused.
	#[pallet::storage]
	type ElectionsActive<T: Config<I>, I: 'static = ()> = StorageValue<_, bool, ValueQuery>;

	// ---------------------------------------------------------------------------------------- //

	/// Implements traits to allow electoral systems to read/write an Election's details.
	struct ElectionAccess<T: Config<I>, I: 'static> {
		election_identifier:
			ElectionIdentifier<<T::ElectoralSystem as ElectoralSystem>::ElectionIdentifierExtra>,
		_phantom: core::marker::PhantomData<(T, I)>,
	}
	impl<T: Config<I>, I: 'static> ElectionAccess<T, I> {
		fn new(
			election_identifier: ElectionIdentifier<
				<T::ElectoralSystem as ElectoralSystem>::ElectionIdentifierExtra,
			>,
		) -> Self {
		...
		fn unique_monotonic_identifier(&self) -> UniqueMonotonicIdentifier {
			self.election_identifier.into()
		}
	}
	impl<T: Config<I>, I: 'static> ElectionReadAccess<T::ElectoralSystem> for ElectionAccess<T, I> {
		fn settings(
			&self,
		) -> Result<<T::ElectoralSystem as ElectoralSystem>::ElectoralSettings, CorruptStorageError>
		{
			...
			let settings_boundary = settings_boundaries
				.iter()
				.rev()
				.find(|settings_boundary| **settings_boundary <= self.unique_monotonic_identifier())
				.ok_or(CorruptStorageError)?;
			ElectoralSettings::<T, I>::get(settings_boundary).ok_or(CorruptStorageError)
		}
		fn properties(
			&self,
		) -> Result<<T::ElectoralSystem as ElectoralSystem>::ElectionProperties, CorruptStorageError>
		{
	...
	impl<T: Config<I>, I: 'static> ElectionWriteAccess<T::ElectoralSystem> for ElectionAccess<T, I> {
		fn set_state(
			&mut self,
		...
		> {
			...
			Ok(
					...
					Some(epoch_index)
				{
					...
					let current_authorities_count: AuthorityCount =
						current_authorities.len().try_into().map_err(|_| CorruptStorageError)?;
					...
					}).filter_map_ok(|(properties, partial_vote_or_vote)| match partial_vote_or_vote {
						PartialOrVote::Vote(vote) => Some((properties, vote)),
						_ => None,
					})
					...
					ElectionConsensusHistory::<T, I>::set(
						...
						match &option_new_consensus {
							...
							None => option_consensus_history.as_ref().map(|consensus_history| {
								ConsensusHistory {
	...
	struct MultiElectionAccess<T: Config<I>, I: 'static> {
		_phantom: core::marker::PhantomData<(T, I)>,
	}
	impl<T: Config<I>, I: 'static> MultiElectionAccess<T, I> {
		fn new() -> Self {
			Self { _phantom: Default::default() }
		}
	}
	impl<T: Config<I>, I: 'static> MultiElectionReadAccess<T::ElectoralSystem>
		for MultiElectionAccess<T, I>
	{
	...
	impl<T: Config<I>, I: 'static> MultiElectionWriteAccess<T::ElectoralSystem>
		for MultiElectionAccess<T, I>
	{
	...
	mod election_bitmap_components {
		use super::{
			BitmapComponents, Config, CorruptStorageError, Pallet, UniqueMonotonicIdentifier,
		};
		use crate::{electoral_system::ElectoralSystem, vote_storage::VoteStorage};
		use bitvec::prelude::*;
		use cf_primitives::{AuthorityCount, EpochIndex};
		...
		pub(super) struct ElectionBitmapComponents<T: Config<I>, I: 'static> {
			epoch: EpochIndex,
			bitmaps: Vec<(
				<<T::ElectoralSystem as ElectoralSystem>::Vote as VoteStorage>::BitmapComponent,
		...
		impl<T: Config<I>, I: 'static> ElectionBitmapComponents<T, I> {
			...
			>(
				...
				if ALWAYS_STORE_AFTER_CLOSURE || updated {
					...
					BitmapComponents::<T, I>::set(
						unique_monotonic_identifier,
						if this.bitmaps.is_empty() { None } else { Some(this) }, /* TODO: Check
						                                                          * None
						                                                          * will remove */
					);
				}
			...
			pub(super) fn add(
				...
				bitmap_component: <<T::ElectoralSystem as ElectoralSystem>::Vote as VoteStorage>::BitmapComponent,
				unique_monotonic_identifier: UniqueMonotonicIdentifier,
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn vote(
			...
			election_identifier: ElectionIdentifier<
				<T::ElectoralSystem as ElectoralSystem>::ElectionIdentifierExtra,
			>,
			partial_vote_or_vote: PartialOrVote<
				<<T::ElectoralSystem as electoral_system::ElectoralSystem>::Vote as VoteStorage>::PartialVote,
				<<T::ElectoralSystem as electoral_system::ElectoralSystem>::Vote as VoteStorage>::Vote,
			>,
		) -> DispatchResult {
			...
			let (partial_vote, option_vote) = match partial_vote_or_vote {
				PartialOrVote::PartialVote(partial_vote) => {
					(partial_vote, None)
				},
				PartialOrVote::Vote(vote) => {
					(
		...
		pub fn update_settings(
			...
			Self::ensure_governance(origin, ignore_corrupt_storage, false)?;
		...
		pub fn pause_elections(origin: OriginFor<T>) -> DispatchResult {
			Self::ensure_governance(origin, CorruptStorageAdherance::Ignore, false)?;
			ElectionsActive::<T, I>::set(false);
			Ok(())
		}
		...
		pub fn unpause_elections(
			...
			Self::ensure_governance(origin, CorruptStorageAdherance::Heed, true)?;
			ensure!(
		...
		pub fn clear_all_votes(
			...
			Self::ensure_governance(
				origin,
				ignore_corrupt_storage,
				// Force governance to pause ahead of time as this extrinsic may pause voting
				// anyway due to corruption by partial deletion, so governance should be willing to
				// pause the elections before resorting to this.
				true,
			)?;
		...
		pub fn validate_storage(origin: OriginFor<T>) -> DispatchResult {
			Self::ensure_governance(origin, CorruptStorageAdherance::Ignore, true)?;

			CorruptStorage::<T, I>::set(false);
			Ok(())
		}
	}
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		fn recheck_contributed_to_consensuses(
				...
				ElectionConsensusHistoryUpToDate::<T, I>::iter_keys().collect::<Vec<_>>()
			{
				...
				.is_some_and(|(_, partial_vote_or_vote)| {
					matches!(partial_vote_or_vote, PartialOrVote::Vote(_))
				}) {
					ElectionConsensusHistoryUpToDate::<T, I>::remove(unique_monotonic_identifier);
				}
			}
		...
		fn take_vote_and_then<
			...
			F: for<'a> FnOnce(
				Option<(
					<<T::ElectoralSystem as ElectoralSystem>::Vote as VoteStorage>::Properties,
					PartialOrVote<
						<<T::ElectoralSystem as ElectoralSystem>::Vote as VoteStorage>::PartialVote,
		...
		fn is_vote_desired(
		...
		) -> Result<Option<core::time::Duration>, DispatchError> {
			...
			Ok(Self::handle_corrupt_storage(
				<T::ElectoralSystem as ElectoralSystem>::is_vote_desired(
					election_identifier,
					&ElectionAccess::<T, I>::new(election_identifier),
					{
						let mut contains_timed_out_shared_data_references = false;

						Self::handle_corrupt_storage(Pallet::<T, I>::get_vote(
							epoch_index,
							unique_monotonic_identifier,
							&authority,
							authority_index,
		...
		fn ensure_current_authority(
			...
			ensure!(ElectionsActive::<T, I>::get(), Error::<T, I>::Paused);
		...
		fn ensure_governance(
			...
			if matches!(ignore_corrupt_storage, CorruptStorageAdherance::Heed) {
				ensure!(!CorruptStorage::<T, I>::get(), Error::<T, I>::CorruptStorage);
			}
			if force_paused {
				ensure!(!ElectionsActive::<T, I>::get(), Error::<T, I>::NotPaused);
			}
			Ok(())
		}
	...
	impl<T: Config<I>, I: 'static> Hooks<BlockNumberFor<T>> for Pallet<T, I> {
		fn on_finalize(block_number: BlockNumberFor<T>) {
			if CorruptStorage::<T, I>::get() {
				Self::deposit_event(Event::<T, I>::CorruptStorage);
			} else if ElectionsActive::<T, I>::get() {
				let election_identifiers =
					...
					sp_core::U256::zero()
				{
					let minimum_election_identifiers = election_identifiers
						.iter()
						.copied()
						.map(UniqueMonotonicIdentifier::from)
						.min()
						.unwrap_or_default();
					let mut settings_boundaries =
						ElectoralSettings::<T, I>::iter_keys().collect::<Vec<_>>();
					settings_boundaries.sort();
					for setting_boundary in &settings_boundaries[..settings_boundaries[..]
				...
				// TODO: Cleanup IncludedAuthorities

				let _ = Self::handle_corrupt_storage(T::ElectoralSystem::on_finalize(
					election_identifiers,
					&mut MultiElectionAccess::<T, I>::new(),
					&(),
				));
			}
		}
